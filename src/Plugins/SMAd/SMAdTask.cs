    ...
    public sealed class SMAdTask : QTPServiceBase
    {
        ...
        private async Task<bool> RunMainFlowAsync(WorkerRunContext ctx, CancellationToken token)
        {
            for (ctx.PvIndex = 1; ctx.PvIndex <= ctx.Config.TotalPV; ctx.PvIndex++)
            {
                ...
                {
                    LogWriteLine($"{this.Title}:ExecuteWorker:ʼ ExecuteHomepageTriggerAsync");
                    var homepageSw = Stopwatch.StartNew();
                    var homepageOk = await ExecuteHomepageTriggerAsync(ctx, entry.QueryWord, token);
                    homepageSw.Stop();
                    LogWriteLine($"{this.Title}:ExecuteWorker:ExecuteHomepageTriggerAsync ʱ {homepageSw.Elapsed.TotalSeconds:N2} 룬={homepageOk}");
                    if (!homepageOk)
                        continue;
                }
                else
                {
                    LogWriteLine($"{this.Title}:ExecuteWorker:ʼ ScrollWithTimeoutAsync");
                    var scrollSw = Stopwatch.StartNew();
                    scrollSw.Stop();
                    LogWriteLine($"{this.Title}:ExecuteWorker:ScrollWithTimeoutAsync ʱ {scrollSw.Elapsed.TotalSeconds:N2} ");
                LogWriteLine($"{this.Title}:ExecuteWorker:ʼ DetectAndUploadAdWordsAsync");
                var adsSw = Stopwatch.StartNew();
                adsSw.Stop();
                LogWriteLine($"{this.Title}:ExecuteWorker:DetectAndUploadAdWordsAsync ʱ {adsSw.Elapsed.TotalSeconds:N2} 룬={adsOk}");
                LogWriteLine($"{this.Title}:ExecuteWorker:ʼ DecideJumpClickAsync");
                var decideJumpSw = Stopwatch.StartNew();
                decideJumpSw.Stop();
                LogWriteLine($"{this.Title}:ExecuteWorker:DecideJumpClickAsync ʱ {decideJumpSw.Elapsed.TotalSeconds:N2} 룬JumpClick={ctx.JumpClick}");
                    LogWriteLine($"{this.Title}:ExecuteWorker:ʼ TryExecuteJumpClickAsync");
                    var jumpClickSw = Stopwatch.StartNew();
                    jumpClickSw.Stop();
                    LogWriteLine($"{this.Title}:ExecuteWorker:TryExecuteJumpClickAsync ʱ {jumpClickSw.Elapsed.TotalSeconds:N2} 룬Flow={clickFlow}");
