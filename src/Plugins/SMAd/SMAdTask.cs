...
namespace QTP.Plugins
{
    public sealed class SMAdTask : QTPServiceBase
    {
        ...
        private static bool IsEventOccurring(double probability)
        {
        ...
        public override async Task<(bool, bool, int)> ExecuteWorkerAsync(string uniqueId, JObject taskArgs, CancellationToken token)
        {
            ...
            await using var browser = await ConnectOverCDPWithRetryAsync(playwright, endpoint, linkedCts.Token);
            if (browser == null)
            {
            ...
            task_sleep:
                {
                    ...
                    if (totalPV > 1)
                    {
                        if (jumpClick && page_trigger_click && !pvsTriggerOne)
                        {
                            goto redo_pv;
                        }
                    }
