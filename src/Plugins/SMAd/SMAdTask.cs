    ...
    public sealed class SMAdTask : QTPServiceBase
    {
        ...
        private async Task<bool> RunMainFlowAsync(WorkerRunContext ctx, CancellationToken token)
        {
            for (ctx.PvIndex = 1; ctx.PvIndex <= ctx.Config.TotalPV; ctx.PvIndex++)
            {
                ...
                {
                    LogWriteLine($"{this.Title}:ExecuteWorker:曝光进入页面停留{((ctx.Config.PageLoadedDelayMs - ctx.DelayAfterGotoMs) / 1000.0):N2}秒");
                    token.ThrowIfCancellationRequested();
                    await ScrollWithTimeoutAsync(ctx.Page!, ctx.CdpManager!, Math.Abs(ctx.Config.PageLoadedDelayMs - ctx.DelayAfterGotoMs));
                }
        ...
        private async Task<bool> NavigateToEntryAsync(WorkerRunContext ctx, string url, CancellationToken token)
        {
            ...
            ctx.DelayAfterGotoMs = CommonHelper.RandomRange(3000, 5000);
            await Task.Delay(ctx.DelayAfterGotoMs, token);
        ...
        private async Task TryHandle1688RecommendWordsAsync(WorkerRunContext ctx, CancellationToken token)
        {
            ...
            try
            {
                ...
                await TouchPageScrollUp(ctx.Page!, ctx.CdpSession!);
        ...
        private async Task HandleJdActivePageAsync(WorkerRunContext ctx, CancellationToken token)
        {
            token.ThrowIfCancellationRequested();
        ...
        private sealed class WorkerRunContext
        {
            ...
            public int DelayAfterGotoMs { get; set; }
