    ...
    public sealed class SMAdTask : QTPServiceBase
    {
        ...
        private async Task<bool> RunMainFlowAsync(WorkerRunContext ctx, CancellationToken token)
        {
            for (ctx.PvIndex = 1; ctx.PvIndex <= ctx.Config.TotalPV; ctx.PvIndex++)
            {
                ...
                {
                    var scrollStageStartMs = Environment.TickCount64;
                    LogWriteLine($"[PWTRACE][FLOW] Stage=ScrollWithTimeout Start durationMs={Math.Abs(ctx.Config.PageLoadedDelayMs)}");
                    LogWriteLine($"[PWTRACE][FLOW] Stage=ScrollWithTimeout End elapsedMs={Environment.TickCount64 - scrollStageStartMs}");
                var detectStageStartMs = Environment.TickCount64;
                LogWriteLine($"[PWTRACE][FLOW] Stage=DetectAndUploadAdWords Start query={entry.QueryWord ?? string.Empty}");
                LogWriteLine($"[PWTRACE][FLOW] Stage=DetectAndUploadAdWords End elapsedMs={Environment.TickCount64 - detectStageStartMs}, adsOk={adsOk}, pageAdsCount={ctx.PageAdsCount}");
                var decideStageStartMs = Environment.TickCount64;
                LogWriteLine("[PWTRACE][FLOW] Stage=DecideJumpClick Start");
                LogWriteLine($"[PWTRACE][FLOW] Stage=DecideJumpClick End elapsedMs={Environment.TickCount64 - decideStageStartMs}, jumpClick={ctx.JumpClick}, pageTriggerClick={ctx.PageTriggerClick}");
        ...
        private async Task<bool> ExecuteHomepageTriggerAsync(WorkerRunContext ctx, string? q, CancellationToken token)
        {
            ...
            LogWriteLine($"[PWTRACE][ADS] Detect start pageAdsCount={ctx.PageAdsCount}, url={ctx.Page!.Url}");
        ...
        private async Task<bool> DetectAndUploadAdWordsAsync(WorkerRunContext ctx, string? q, CancellationToken token)
        {
            ...
            {
                LogWriteLine($"[PWTRACE][ADS] Detect end ad1688={ad1688}, adOther={adOther}, domains1688={domains1688.Count}, domainsOther={domainsOther.Count}, noTrigger1688=true");
            LogWriteLine($"[PWTRACE][ADS] Detect end ad1688={ad1688}, adOther={adOther}, domains1688={domains1688.Count}, domainsOther={domainsOther.Count}, brands={brandsSet.Count}");
            LogWriteLine($"[PWTRACE][CLICK] Decide start taskId={ctx.Config.TaskId}, clickRate={clickRate}");
            {
                LogWriteLine("[PWTRACE][CLICK] Decide skip because clickRate<=0");
            }
            LogWriteLine($"[PWTRACE][CLICK] Decide end ctr={(ctr * 100):N2}%, jumpClick={ctx.JumpClick}");
